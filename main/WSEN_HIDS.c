...
int WSEN_HIDS_init()
{
    ...
    // WSEN_HIDS init
    // check Communikation via ID check
        uint8_t DevID = 0;

        CHECK(ReadReg(0x0f,1,&DevID));
        // printf("I2C DevID %02x\n", did);
        if(DevID == 0xBC ){
            printf("Temp-Sensor Communication successful! \n");
        }
        else{
            printf("Temp-Sensor Communication failed! \n");
        }

    // Enable continuous operation mode Enable BDU bit Select output data rate: 1 Hz
        uint8_t  CTRL_1;

        
        CHECK(ReadReg(HIDS_CTRL_REG_1,1,&CTRL_1));
        CTRL_1 = (CTRL_1 | 0b10000101);
        CHECK(WriteReg(HIDS_CTRL_REG_1 , 1, &CTRL_1));



    // Read calibration registers
        uint8_t  re0x30;
        CHECK(ReadReg(HIDS_H0_RH_X2,1,&re0x30));
        uint8_t  re0x3f;
        CHECK(ReadReg(HIDS_T1_OUT_H,1,&re0x3f));

    // Enable DRDY_EN bit
        uint8_t  CTRL_3;
        CHECK(ReadReg(HIDS_CTRL_REG_3,1,&CTRL_3));
        CTRL_3 = (CTRL_3 | 0b00000100);
        CHECK(WriteReg(HIDS_CTRL_REG_3 , 1, &CTRL_3));
